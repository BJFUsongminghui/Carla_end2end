...
import left_end_to_end_carla
import time
import os

actions = left_end_to_end_carla.actions
...
class DQNAgent:
    ...
    def __init__(self, action_num=3):
        ...
        if os.path.exists('left_models/model.pkl'):
            self.model.load_state_dict(torch.load('left_models/model.pkl'))
        if os.path.exists('left_models/target_model.pkl'):
            self.target_model.load_state_dict(torch.load('left_models/target_model.pkl'))
        self.replay_memory = deque(maxlen=REPLAY_MEMORY_SIZE)
        self.tensorboard = SummaryWriter('left_end_to_end_logs')
...
def main():
    if not os.path.isdir('left_models'):
        os.makedirs('left_models')
    env = left_end_to_end_carla.CarEnv()
    ...
    for episode in range(EPISODES):
        ...
        if episode_reward >= MIN_REWARD:
            torch.save(agent.model.state_dict(), f'left_models/{episode_reward:>7.2f}reward.model')

        if episode % 10 == 0:
            print("episodes {},  reward is {} spend time is {} ".format(episode, episode_reward,
                                                                        time.time() - episode_start))
            torch.save(agent.model.state_dict(), 'left_models/model.pkl')
            torch.save(agent.target_model.state_dict(), 'left_models/target_model.pkl')
