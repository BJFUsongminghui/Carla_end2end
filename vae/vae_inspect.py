...
class GETVAE():
    ...
    def get_code(self,rgb):
        input = torch.Tensor([rgb]).float() / 255
        x, mu, logvar, z = self.model(input)
        return z,x

def preprocess_rgb_frame(frame):
    frame = frame[:, :, :3]
    matplotlib.image.imsave('out2.png', frame)
    # RGBA -> RGB
...
if __name__ == '__main__':
    model = VAE()
    model.load_state_dict(torch.load('model/125model.pt'))
    image=get_data()
    x=image[0].reshape((88, 200,3))
    cv2.imwrite('out1.png', x)
    input = torch.Tensor([image[0]]).float()
    x, mu, logvar, z = model(input)
    frame = x.reshape((88, 200,3))
    frame=frame.detach().numpy()*255
